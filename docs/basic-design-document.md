...
## 1.2. 試作品の概要
本システムの試作品は、冷蔵庫の中の食材情報を活用するディスプレイ表示と、スマホを用いた外出モチベーション向上のためのアプリケーション開発とする。
冷蔵庫内の食材の画像認識はすでにある[（パナソニック "Live Pantry"）](https://panasonic.jp/reizo/function/camera.html)ので優先順位は低く、ここから得られる情報の見せ方に焦点を当てる。

冷蔵庫のディスプレイではおすすめするレシピを３種類ほど表示し、ユーザーの選択によって、選ばれたレシピと必要な食材の表示を行う。
さらに、周辺のスーパーマーケットの情報を表示した地図を表示する。特定の時間になったときに外出を促す。
その時間までにレシピの選択が行われていなければ、レシピの選択を促した後、外出を促す。

サーバーでは、レシピ用データベースを保持しており、冷蔵庫側からのリクエストに応じて、レシピと必要な食材を送信する。

ディスプレイに表示する情報は現在の庫内の食材情報・レシピ・レシピに使用する食材・スーパーマーケットの位置とする。

試作品に含まれるスマートフォンアプリケーションは、冷蔵庫ディスプレイと連携し、ユーザーが外出時に持ち歩くことができる情報サポートを提供する。
...
- レシピと食材を保存したDB
## 3.6. スマホアプリ
- 外出ナビゲーション：選択されたスーパーマーケットまでの最適なルートを提供し、歩数や消費カロリーをリアルタイムで表示する。
- 健康とモチベーションの追跡：外出に関連する歩数やカロリー消費を追跡し、目標達成に向けた進捗状況をユーザーにフィードバックする。また、外出を完了すると特定の健康目標に対する報酬や進捗をアプリが表示する。
- インタラクティブなショッピングリスト：冷蔵庫から送られた食材リストを利用して、ユーザーがスーパーマーケットで必要な食材を効率的に購入できるようにする。リストは購入した項目をチェックオフすることが可能で、購入忘れの防止に役立つ。
- ゲーミフィケーション要素：歩数や活動に応じてポイントが加算され、特定のレベルに達するとバーチャルの報酬や認証バッジが解除されることで、ユーザーのモチベーション維持を促進する。
# 4. 非機能要件
## 4.1. ディスプレイ
